type Mode = "education" | "workforce";
type View = "overview" | "people" | "detail" | "policy" | "insights" | "ops";
type Status = "Online" | "Idle" | "Alert" | "Review";
type ScreenKind = "focus" | "docs" | "idle" | "alert" | "locked";
...
let mode: Mode = "education";
let view: View = "overview";
let selectedIndex = 0;
let activeCommand = "Focus message";
let reviewRecorded = false;
let liveHealth: HealthPayload | null = null;
...
const datasets: Record<Mode, DemoDataset> = {
  education: {
...
function currentDataset(): DemoDataset {
...
function healthTone(status: HealthPayload["overall_status"] | undefined): string {
  if (status === "ok") return "green";
  if (status === "down") return "red";
  return "amber";
}

async function refreshLiveHealth(): Promise<void> {
  healthChecked = true;
  try {
...
function statusTone(status: Status): string {
  ...
  if (status === "Idle" || status === "Review") return "amber";
  return "";
}
...
async function loadAdapters(): Promise<void> {
  const [gsapModule, lenisModule] = await Promise.all([
    importOptional<Record<string, unknown>>(CDN.gsap),
    importOptional<Record<string, unknown>>(CDN.lenis),
  ]);

  adapters.gsap = (gsapModule?.gsap ?? gsapModule?.default) as GsapApi | undefined;
  adapters.Lenis = (lenisModule?.default ?? lenisModule?.Lenis) as AdapterState["Lenis"];
...
function renderPeopleGrid(): void {
  ...
  `).join("");
  all<HTMLElement>("[data-person-index]").forEach((card) => {
...
function renderPeopleTable(): void {
  const query = el<HTMLInputElement>("people-search").value.trim().toLowerCase();
  ...
  all<HTMLButtonElement>("[data-open-person]").forEach((button) => {
...
function renderDetail(): void {
  ...
  const detailStatus = el<HTMLSpanElement>("detail-status");
...
function setView(nextView: View): void {
  view = nextView;
  all<HTMLButtonElement>("[data-view]").forEach((button) => button.classList.toggle("active", button.dataset.view === nextView));
  all<HTMLElement>(".view").forEach((section) => section.classList.remove("active"));
...
function openCommand(command: string): void {
  activeCommand = command;
  const person = selectedPerson();
  el("modal-title").textContent = command;
  el("modal-subtitle").textContent = `${person.name} · ${person.id}`;
  el<HTMLSelectElement>("command-scope").value = command === "Broadcast" ? "Current group" : "Selected person";
  el<HTMLInputElement>("command-payload").value = command === "Push URL"
...
function recordGovernanceReview(): void {
  reviewRecorded = true;
  renderAll();
...
function bindEvents(): void {
  ...
  all<HTMLButtonElement>("[data-mode]").forEach((button) => {
    button.addEventListener("click", () => {
      mode = button.dataset.mode as Mode;
      selectedIndex = 0;
      all<HTMLButtonElement>("[data-mode]").forEach((item) => item.classList.toggle("active", item === button));
      renderAll();
      setView(view);
