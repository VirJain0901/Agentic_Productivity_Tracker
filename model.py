...
class SimpleEmployeeMonitoringAgent:
    ...
    def __init__(self, employee_id, manager_config):
        self.employee_id = str(employee_id)  # Ensure string
...
class ManagerDashboard:
    ...
    def get_employee_summary(self, all_data):
        ...
        # Add suspicious_files only if it exists
...
# =====================================================
# NEW MODEL TRAINING & VALIDATION FUNCTIONS
# =====================================================
...
def train_and_validate_model():
    ...
    # Test on custom scenarios
    print("\n" + "="*60)
    print("🧪 TESTING ON CUSTOM SCENARIOS")
    print("="*60)
    ...
    test_scenarios = [
        {
            ...
            'metrics': {
                'productive_app_usage': 250,
                'unproductive_app_usage': 20,
                'idle_time': 10,
                'app_switch_count': 5,
                'deleted_files': 0,
                'focus_score': 0.95,
                'task_completion_rate': 0.98,
                'productive_ratio': 0.92
            }
        },
        {
            'name': 'Poor Employee',
            'metrics': {
                'productive_app_usage': 30,
                'unproductive_app_usage': 180,
                'idle_time': 90,
                'app_switch_count': 25,
                'deleted_files': 12,
                'focus_score': 0.35,
                'task_completion_rate': 0.40,
                'productive_ratio': 0.14
            }
        },
        {
            'name': 'Average Employee',
            'metrics': {
                'productive_app_usage': 120,
                'unproductive_app_usage': 80,
                'idle_time': 45,
                'app_switch_count': 12,
                'deleted_files': 3,
                'focus_score': 0.65,
                'task_completion_rate': 0.70,
                'productive_ratio': 0.60
            }
        },
        {
            'name': 'Suspicious Behavior',
            'metrics': {
                'productive_app_usage': 150,
                'unproductive_app_usage': 40,
                'idle_time': 15,
                'app_switch_count': 8,
                'deleted_files': 25,
                'focus_score': 0.75,
                'task_completion_rate': 0.80,
                'productive_ratio': 0.79
            }
        }
    ]
    
    for scenario in test_scenarios:
        test_df = pd.DataFrame([list(scenario['metrics'].values())], 
                               columns=features)
        
        pred = rf_model.predict(test_df)[0]
        proba = rf_model.predict_proba(test_df)[0][1]
        anomaly = iso_forest.predict(test_df)[0]
        
        status = "✅ Productive" if pred == 1 else "❌ Unproductive"
        anomaly_status = "⚠️ ANOMALY" if anomaly == -1 else "Normal"
        
        print(f"\n📊 {scenario['name']}:")
        print(f"   Prediction: {status} ({proba:.1%} confidence)")
        print(f"   Status: {anomaly_status}")
        
        if scenario['name'] == 'Suspicious Behavior' and anomaly == -1:
            print(f"   ✅ Correctly flagged as anomaly!")
        elif scenario['name'] == 'Suspicious Behavior' and anomaly != -1:
            print(f"   ⚠️ Missed anomaly detection!")
...
def compare_models():
    ...
    # Ask if user wants to train the best model on full data
    answer = input(f"\n💡 Do you want to train the {best_model['Model']} on full data? (y/n): ").strip().lower()
    if answer == 'y':
        print(f"\n[*] Training {best_model['Model']} on full dataset...")
        if best_model['Model'] == 'Random Forest':
            ...
            best_model_obj = GradientBoostingClassifier(n_estimators=200, random_state=42)
        elif best_model['Model'] == 'Logistic Regression':
            best_model_obj = LogisticRegression(random_state=42, max_iter=1000)
        else:
            best_model_obj = KNeighborsClassifier(n_neighbors=5)
        
        best_model_obj.fit(X, y)
        joblib.dump(best_model_obj, 'productivity_model_enhanced.pkl')
        print(f"✅ Best model saved as 'productivity_model_enhanced.pkl'")
...
# =====================================================
# MAIN PROGRAM WITH UPDATED MENU
# =====================================================

def run_employee_mode():
    """Run as employee agent - RETURNS TO MENU when done"""
    print("\n" + "="*60)
    print("🖥️  EMPLOYEE MONITORING AGENT WITH FILE DETECTION")
    print("="*60)
...
def run_manager_mode():
    """Run as manager - RETURNS TO MENU after actions"""
    print("\n" + "="*60)
    print("👔 MANAGER DASHBOARD WITH SECURITY MONITORING")
    print("="*60)
    ...
    while True:
        ...
        if choice == '1':
            ...
            # Ensure emp_id is string for comparison
            emp_id = str(emp_id)
            ...
            if not emp_data.empty:
                ...
                emp_data.to_csv(f"employee_{emp_id}_details.csv", index=False)
                print(f"\n✅ Details exported to employee_{emp_id}_details.csv")
            else:
                print(f"❌ No data found for Employee {emp_id}")
                print(f"Available employees: {all_data['employee_id'].unique().tolist()}")
            input("\nPress Enter to continue...")
        elif choice == '4':
            ...
            if not security_alerts.empty:
                ...
                security_alerts.to_csv("security_alerts_export.csv", index=False)
                print("\n✅ Security alerts exported to security_alerts_export.csv")
            else:
...
def run_demo():
    """Run complete demo - RETURNS TO MENU when done"""
    print("\n" + "="*60)
...
def run_doc_generation():
    """Generate documentation and reports - RETURNS TO MENU"""
    print("\n" + "="*60)
...
# =====================================================
# MAIN LOOP - UPDATED WITH NEW OPTIONS
# =====================================================
